...
CREATE OR REPLACE FUNCTION public._CDB_Setup_FDW(fdw_name text, config json)
RETURNS void
AS $$
DECLARE
  row record;
...
BEGIN
  ...
  FOR row IN SELECT p.key, p.value from lateral json_each_text(config->'server') p
    ...
    SELECT public.CDB_Organization_Member_Group_Role_Member_Name() INTO org_role;
...
CREATE OR REPLACE FUNCTION public._CDB_Setup_FDWS()
RETURNS VOID AS 
$$
DECLARE
row record;
BEGIN
  FOR row IN SELECT p.key, p.value from lateral json_each(public.CDB_Conf_GetConf('fdws')) p LOOP
      EXECUTE 'SELECT public._CDB_Setup_FDW($1, $2)' USING row.key, row.value;
    END LOOP;
  END
$$
LANGUAGE PLPGSQL;
...
CREATE OR REPLACE FUNCTION public._CDB_Setup_FDW(fdw_name text)
  RETURNS void AS
$BODY$
DECLARE
config json;
BEGIN
  SELECT p.value FROM LATERAL json_each(public.CDB_Conf_GetConf('fdws')) p WHERE p.key = fdw_name INTO config;
  EXECUTE 'SELECT public._CDB_Setup_FDW($1, $2)' USING fdw_name, config;
END
$BODY$
LANGUAGE plpgsql VOLATILE;

CREATE OR REPLACE FUNCTION public.CDB_Add_Remote_Table(source text, table_name text)
  RETURNS void AS
$$
BEGIN
  PERFORM public._CDB_Setup_FDW(source);
...
CREATE OR REPLACE FUNCTION public.CDB_Get_Foreign_Updated_At(foreign_table regclass)
...
CREATE OR REPLACE FUNCTION public._cdb_dbname_of_foreign_table(reloid oid)
RETURNS TEXT AS $$
...
CREATE OR REPLACE FUNCTION public.CDB_QueryTables_Updated_At(query text)
...
AS $$
    ...
    fqtn AS (
      SELECT
        (CASE WHEN c.relkind = 'f' THEN public._cdb_dbname_of_foreign_table(query_tables_oid.reloid)
    ...
    SELECT fqtn.dbname, fqtn.schema_name, fqtn.table_name,
      (CASE WHEN relkind = 'f' THEN public.CDB_Get_Foreign_Updated_At(reloid)
...
-- It is aware of foreign tables
-- It assumes the local (schema_name, table_name) map to the remote ones with the same name
CREATE OR REPLACE FUNCTION public.CDB_Last_Updated_Time(tables text[])
